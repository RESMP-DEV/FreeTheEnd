...
private:
  ...
  Buffer player_buffer_;     // Small player buffer matching dragon_fight shader
  Buffer dragon_buffer_;     // Dragon state for dragon_fight shader
  Buffer crystal_buffer_;    // End crystals for dragon_fight shader
  Buffer game_state_buffer_; // Game state for dragon_fight shader
  Buffer entity_buffer_;
  Buffer mob_counts_buffer_; // Per-env mob count
  ...
  std::vector<uint32_t>
      stage_progress_; // Per-env bitmask of completed objectives

  // Chunk cache: key = (env_id << 32) | ((chunk_x & 0xFFFF) << 16) | (chunk_z &
  // 0xFFFF)
