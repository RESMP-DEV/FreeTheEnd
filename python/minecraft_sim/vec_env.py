...
class SB3VecFreeTheEndEnv(SB3VecEnv):
    ...
    def __init__(
        ...
        if not HAS_SB3:
            raise ImportError("stable_baselines3 is required for SB3VecFreeTheEndEnv")
        ...
        observation_space = spaces.Box(low=0.0, high=1.0, shape=(self._obs_size,), dtype=np.float32)
        action_space = spaces.Discrete(self._num_actions)

        # Initialize SB3 VecEnv base class
        super().__init__(
            num_envs=num_envs, observation_space=observation_space, action_space=action_space
        )
