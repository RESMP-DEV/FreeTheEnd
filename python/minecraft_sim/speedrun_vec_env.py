...
class SpeedrunVecEnv:
    ...
    def __init__(
        ...
        max_ticks_per_episode: int | None = None,
        use_multistage_simulator: bool = False,
    ) -> None:
        if mc189_core is None:
            raise ImportError(
        ...
        self.render_mode = None  # Required by VecNormalize
        self._use_multistage = use_multistage_simulator

        # MultistageSimulator uses 256-float observations
        if use_multistage_simulator:
            self._obs_size = 256
        else:
        ...
        if use_multistage_simulator:
            # Use MultistageSimulator for proper stage-aware simulation
            config = mc189_core.MultistageConfig()
            config.num_envs = num_envs
            config.initial_stage = mc189_core.GameStage.BASIC_SURVIVAL
            config.auto_advance_stage = False  # We manage curriculum in Python
            if shader_dir is not None:
                config.shader_dir = str(shader_dir)
            else:
                default_shader_dir = Path(__file__).parent.parent.parent / "cpp" / "shaders"
            ...
            self.sim = mc189_core.MultistageSimulator(config)
        else:
            # Use original MC189Simulator
            config = mc189_core.SimulatorConfig()
            config.num_envs = num_envs
            if shader_dir is not None:
                config.shader_dir = str(shader_dir)
            else:
                # Default: look for cpp/shaders relative to the Python package
                default_shader_dir = Path(__file__).parent.parent.parent / "cpp" / "shaders"
                if default_shader_dir.exists():
                    config.shader_dir = str(default_shader_dir)

            # Load only the shaders needed for the initial stage (if backend supports it)
            # For END_FIGHT, use the monolithic dragon_fight shader (shader_set empty)
            if hasattr(config, "shader_set") and initial_stage != StageID.END_FIGHT:
        ...
        # Max episode length (for training - ensures episodes complete)
        self._max_ticks_per_episode = max_ticks_per_episode

        # Previous observations for delta-based reward shaping
        self._prev_obs: NDArray[np.float32] | None = None

        # Per-environment milestone tracking for one-time bonuses
        self._env_milestones: list[set[str]] = [set() for _ in range(num_envs)]
    ...
    def reset(
        ...
        *,
        ...
        # Get initial observations and store for delta tracking
        obs = self._get_obs()
        self._prev_obs = obs.copy()

        # Reset milestones on full reset
        self._env_milestones = [set() for _ in range(self.num_envs)]
    ...
    def step(
        ...
        # Apply stage-specific reward shaping (uses observation deltas)
        rewards = self._apply_reward_shaping(rewards, obs, self._prev_obs, dones)

        # Update previous observations for next step
        # For done envs, they will reset so we use the new obs
        self._prev_obs = obs.copy()
        ...
        # Check for max episode length (truncation)
        if self._max_ticks_per_episode is not None:
            truncated = self._episode_lengths >= self._max_ticks_per_episode
            # Mark truncated environments as done (for episode completion)
        ...
        for i in range(self.num_envs):
            if dones[i]:
                ...
                # Reset milestones for this environment
                self._env_milestones[i].clear()
    ...
    def _apply_reward_shaping(
        ...
        prev_obs: NDArray[np.float32] | None,
        dones: NDArray[np.bool_],
    ) -> NDArray[np.float32]:
        """Apply stage-specific reward shaping based on observation deltas.

        Observation layout (48 floats, see cpu_backend.cpp):
            [0-2]   position (x, y, z) normalized
            [3-5]   velocity (x, y, z) normalized
            [6-7]   yaw, pitch normalized
            [8]     health (0-1, * 20 = HP)
            [9]     hunger (0-1, * 20)
            [10]    on_ground (0 or 1)
            [11]    attack_ready (0 or 1)
            [12]    weapon_slot
            [13]    arrows (normalized)
            [14]    arrow_charge
            [16]    dragon_health (0-1)
            [24]    dragon_phase
            [28]    can_hit_dragon
            [32]    crystals_remaining (0-1)

        Shaped rewards based on observable changes:
        - Health delta: penalty for damage, small bonus for healing
        - Movement: small exploration bonus
        - Combat engagement: bonus for attack_ready cycling (indicates combat)
        - Dragon damage: bonus for health decrease (End fight)
        - Crystal destruction: bonus when crystals decrease

        Args:
            rewards: Raw rewards from simulator.
            obs: Current observations shape (num_envs, obs_size).
            prev_obs: Previous observations, or None on first step.
        ...
        if prev_obs is None:
            # First step, no deltas available
            return shaped_rewards

        # =====================================================================
        # Global observation-delta rewards (all stages)
        # =====================================================================

        # Health delta (obs[8] is health normalized to [0,1])
        health_curr = obs[:, 8]
        health_prev = prev_obs[:, 8]
        health_delta = health_curr - health_prev

        # Damage taken penalty (health decreased)
        damage_mask = health_delta < -0.01  # At least 0.2 HP lost
        shaped_rewards[damage_mask] += health_delta[damage_mask] * 1.0  # -1.0 per full 20HP

        # Healing bonus (health increased, much smaller)
        heal_mask = health_delta > 0.01
        shaped_rewards[heal_mask] += health_delta[heal_mask] * 0.2

        # Movement bonus (exploration)
        pos_curr = obs[:, :3]
        pos_prev = prev_obs[:, :3]
        pos_delta = pos_curr - pos_prev
        distance = np.sqrt(np.sum(pos_delta**2, axis=1))
        # Small bonus for movement: 0.005 per block moved (capped at 1 block)
        shaped_rewards += np.clip(distance, 0, 0.1) * 0.05
        ...
        for stage_id in np.unique(self._env_stages):
            ...
            if stage == StageID.BASIC_SURVIVAL:
                ...
                # Reward staying alive (no damage taken)
                alive_no_damage = mask & ~dones & (health_delta >= 0)
                shaped_rewards[alive_no_damage] += 0.005

                # Bonus for maintaining high health
                healthy = mask & (health_curr > 0.75)
                shaped_rewards[healthy] += 0.002

                # Combat engagement indicator: attack_ready cycling
                # If attack was ready last step and now isn't, likely attacked
                attack_prev = prev_obs[:, 11]
                attack_curr = obs[:, 11]
                attacked = mask & (attack_prev > 0.5) & (attack_curr < 0.5)
                shaped_rewards[attacked] += 0.02  # Small combat bonus

            elif stage == StageID.RESOURCE_GATHERING:
                # Stage 2: Resource gathering
                # Similar survival bonuses, plus combat
                alive_no_damage = mask & ~dones & (health_delta >= 0)
                shaped_rewards[alive_no_damage] += 0.003

                # Combat bonus
                attack_prev = prev_obs[:, 11]
                attack_curr = obs[:, 11]
                attacked = mask & (attack_prev > 0.5) & (attack_curr < 0.5)
                shaped_rewards[attacked] += 0.03

            elif stage == StageID.NETHER_NAVIGATION:
                # Stage 3: Nether navigation
                # Combat is crucial (blaze hunting)
                attack_prev = prev_obs[:, 11]
                attack_curr = obs[:, 11]
                attacked = mask & (attack_prev > 0.5) & (attack_curr < 0.5)
                shaped_rewards[attacked] += 0.05  # Higher combat bonus

            elif stage == StageID.ENDERMAN_HUNTING:
                # Stage 4: Enderman hunting
                # Combat very important
                attack_prev = prev_obs[:, 11]
                attack_curr = obs[:, 11]
                attacked = mask & (attack_prev > 0.5) & (attack_curr < 0.5)
                shaped_rewards[attacked] += 0.05

            elif stage == StageID.STRONGHOLD_FINDING:
                # Stage 5: Stronghold finding
                # Movement/exploration more important
                shaped_rewards[mask] += np.clip(distance[mask], 0, 0.1) * 0.03

            elif stage == StageID.END_FIGHT:
                # Stage 6: Dragon fight
                # Dragon health delta (obs[16] is dragon_health normalized)
                dragon_curr = obs[:, 16]
                dragon_prev = prev_obs[:, 16]
                dragon_delta = dragon_prev - dragon_curr  # Positive when dragon damaged

                # Bonus for dragon damage
                dragon_damaged = mask & (dragon_delta > 0.001)
                shaped_rewards[dragon_damaged] += dragon_delta[dragon_damaged] * 5.0

                # Crystal destruction bonus (obs[32] is crystals_remaining)
                crystals_curr = obs[:, 32]
                crystals_prev = prev_obs[:, 32]
                crystals_delta = crystals_prev - crystals_curr  # Positive when crystal destroyed

                crystal_destroyed = mask & (crystals_delta > 0.05)
                shaped_rewards[crystal_destroyed] += 3.0  # ~3.0 per crystal

                # Milestone: first dragon hit
                for i in np.where(dragon_damaged)[0]:
                    if "first_dragon_hit" not in self._env_milestones[i]:
                        self._env_milestones[i].add("first_dragon_hit")
                        shaped_rewards[i] += 10.0

                # Milestone: dragon killed (health near 0)
                dragon_killed = mask & (dragon_curr < 0.01) & (dragon_prev > 0.01)
                for i in np.where(dragon_killed)[0]:
                    if "dragon_killed" not in self._env_milestones[i]:
                        self._env_milestones[i].add("dragon_killed")
                        shaped_rewards[i] += 100.0

        return shaped_rewards.astype(np.float32)
...
def make_speedrun_vec_env(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        use_multistage_simulator: Use MultistageSimulator for proper stage handling.
    ...
    Example:
        ...
        >>> env = make_speedrun_vec_env(64, use_multistage_simulator=True)
    """
    ...
    return SpeedrunVecEnv(
        ...
        use_multistage_simulator=use_multistage_simulator,
        **kwargs,
    )
